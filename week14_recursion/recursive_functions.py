...
def binary_search(data: list[int], key: int):
    ...
    def __binary_search(data: list[int], key: int, low: int, high: int) -> int:
        mid: int = (low + high) // 2
        ...
        if key < data[mid]:     # Recursive Case 1
            return __binary_search(data, key, low, mid - 1)
        else:
            return __binary_search(data, key, mid + 1, high)

    return __binary_search(data, key, 0, len(data) - 1)
...
def palindrome(s: str) -> bool:	
    return __palindrome(s, 0, len(s) - 1)

# Recursive helper function
def __palindrome(s: str, low: int, high: int) -> bool:
    if low >= high:         # Base Case 1
        return True
    elif s[low] != s[high]: # Base Case 2
        return False
    else:
        return __palindrome(s, low + 1, high - 1)
...
def print_list(data: list) -> None:
    ...
    def __print_list(data: list, index: int) -> None:
        # Hidden base case
        if index < len(data):
            print(str(data[index]), end=' ')
            __print_list(data, index+1)

    __print_list(data, 0)

def print_2D_list(data: list[list]) -> None:
    # Recursive Helper Function
    def __print_2D_list(data: list[list], 
                      row_num: int, col_num: int) -> None:
        if row_num >= len(data):    # Base Case
            return
        elif col_num >= len(data[row_num]):
            print()
            __print_2D_list(data, row_num + 1, 0)
        else:
            print(data[row_num][col_num], end= ' ')
            __print_2D_list(data, row_num, col_num + 1)
        
    __print_2D_list(data, 0, 0)
