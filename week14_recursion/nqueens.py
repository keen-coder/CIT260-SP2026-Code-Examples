    ...
    if (row_num == len(board)):
        ...
        for col_num in range(len(current_row)):
            if is_safe(board, row_num, col_num):
                ...
                solve(board, num_queens, row_num+1)  # <-- Recursive call
                current_row[col_num] = '*' # This is the 'backtracking' we have to
                                           # undo the move that we made BEFORE
                                           # the recursive call

# Function to check whether the given row and col positions are safe to put a queen
# need to Check the row, column, and both diagonals relative to the given row/col coordinates.
